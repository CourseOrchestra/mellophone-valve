...
def mock_sync_client(monkeypatch):
    def _install(response: httpx.Response) -> List[Dict]:
        ...
        monkeypatch.setattr(mellophone.httpx, "Client", client_factory)
        return calls
...
def mock_async_client(monkeypatch):
    def _install(response: httpx.Response) -> List[Dict]:
        ...
        monkeypatch.setattr(mellophone.httpx, "AsyncClient", async_client_factory)
        return calls
...
def mock_requests_session(monkeypatch):
    def _install(response) -> List[Dict]:
        if mellophone.requests is None:
        ...
        monkeypatch.setattr(mellophone.requests, "Session", session_factory)
        return calls
...
def test_xml_to_json_with_repeated_tags():
    ...
    result = mellophone.xml_to_json(xml)
